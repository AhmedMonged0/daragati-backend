...
// 2. كشط درجات ومواد الطالب بناء على المحافظة ورقم الجلوس
app.get('/api/v1/result', async (req, res) => {
  ...
  try {
    ...
    let cleanName = rawName
      ...
      .replace(/الإسكندرية/g, '')
      .replace(/القاهرة/g, '')
      .replace(/الجيزة/g, '')
      .replace(/الدقهلية/g, '')
      .replace(/الفصل الدراسي الثاني/g, '')
      .replace(/الفصل الدراسي الأول/g, '')
      .replace(/٢٠٢٦/g, '')
      .replace(/2026/g, '')
      .trim();
      ...
      if (label && !label.includes('المدرسة') && !label.includes('الإدارة') && !label.includes('المجموع') && !label.includes('التقدير') && !label.includes('رقم') && !label.includes('الاسم')) {
        ...
        if (value) {
          grades.push({
            subject: label,
            score: value,
            max: maxText !== "0" ? maxText : "مادة أساسية"
          });

          if (!isNaN(scoreNum)) {
            if (label.includes('اللغة العربية') || 
                label.includes('اللغة الانجليزية') || 
                label.includes('اللغة الإنجليزية') ||
                label.includes('مجموع الرياضيات') || 
                label.includes('الدراسات') || 
                label.includes('العلوم')) {
    ...
    if (finalScore === 0 && grades.length === 0) {
      return res.status(404).json({ 
        success: false, 
        message: `عذراً، نتيجة محافظة ${gov} لم تعتمد بعد أو رقم الجلوس غير صحيح.` 
      });
    }
    ...
    res.json({
      success: true,
      result: {
        name: cleanName || "طالب ناجح",
        status: statusText || "ناجح",
    ...
    res.status(404).json({ 
      success: false, 
      message: `عذراً، النتيجة غير متاحة حالياً لمحافظة ${gov} برقم الجلوس هذا.` 
    });
  }
});

// ضبط البورت لبيئة الرفع والإنتاج لايف
const PORT = process.env.PORT || 5000;
app.listen(PORT, () => {
  console.log(`🚀 السيرفر الموزون شغال تمام على بورت: ${PORT}`);
});